...
//변수 **함수(호출,준비) 위에 있어야 함!**
let num2 = 20; //블록 스코프 밖(전역스코프) 전역변수 생성
...
//함수호출
func_z(); //1+undifined -> NaN == Not a Number 약자(숫자가 아니라 계산할 수 없음)
...
function roomB(){//함수 블록 스코프 안
    let drink = '우유'
    console.log(`B방에서 먹는 간식 : ${snack}와 ${drink}`); //전역변수 호출(roomB를 호출해야 가능)
}
...
function cart(product, price, quantity=1){
    let item = '';
    item += `상품명: ${product} `;
    item += `가격: ${price.toLocaleString('ko-ko')}원 `;
    item += `수량: ${quantity}개`;
    console.log(item);
}
...
cafeKiosk(0, 0, 2);
cafeKiosk(1, 1);
//cafeKiosk(undefined, 1); //undefined 호출할때 정의안하겠다는 뜻
cafeKiosk(0, 2, 2);
cafeKiosk(1, 1);
//cafeKiosk(undefined, 1);

function cafeKiosk(temp, menu, num=1){
    console.log(`${hotIce[temp]} ${menuAll[menu]} ${num}잔 주문완료되었습니다.`);
}